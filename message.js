...
client.on("messageCreate", message => {
    ...
    const prefixRegex = new RegExp(`^(<@!?${prefixMention}>|${escapeRegex(prefix)})\\s*`);
    ...
    if (now < expirationTime) {
      ...
      .setTitle(`Nutella | Кулдаун`)
      .setColor(err)
      .setDescription(`Подождите ${timeLeft.toFixed(1)}с\n Перед тем как использовать команду - \`${command.name}\``)
    ...
    let embed = new MessageEmbed()
      ...
      .setColor(err)
      .setDescription(`При выполнении этой команды произошла ошибка. Обратитесь к администрации: \`bug\``)
